...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (media.storage_key.includes('/compatible/')) return response.status(200).json({ optimized: false, alreadyCompatible: true })
    ...
    const newKey = `tv/${company.companyId}/videos/compatible/${randomUUID()}.mp4`
    ...
    }).eq('id', media.id).eq('company_id', company.companyId).select('id').maybeSingle()
