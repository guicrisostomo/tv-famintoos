...
export interface UploadTicket { uploadUrl: string; storageKey: string; publicUrl?: string; expiresAt: string }
export interface R2ExistingObject { key: string; filename: string; publicUrl: string; size: number; lastModified: string | null; type: 'image' | 'video' }
...
export async function requestR2Upload(file: File, mediaType: string): Promise<UploadTicket> {
  ...
  const response = await authenticatedFetch('/api/tv/media/upload-ticket', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: file.name, mimeType: file.type, fileSize: file.size, mediaType }) })
  if (!response.ok) { const detail = await response.text().catch(() => ''); throw new Error(`Cloudflare R2 indisponível: ${detail || `HTTP ${response.status}`}`) }
...
export async function listUnregisteredR2Objects() {
  ...
  const result = await response.json().catch(() => ({})) as { objects?: R2ExistingObject[]; error?: string }
  if (!response.ok) throw new Error(result.error ?? `Falha ao listar mídias do R2 (HTTP ${response.status}).`)
