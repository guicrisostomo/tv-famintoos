...
export function TvPlayer({ companyId, displayId }: { companyId: string, displayId: string }) {
  ...
  const [playerDiagnostics, setPlayerDiagnostics] = useState<TvPlayerDiagnostics>(() => runtime.snapshot())
  ...
  const progressRef = useRef({ itemId: '', value: 0, changedAt: 0, recoveries: 0 })
  const diagnosticMode = ['audio', 'player'].includes(new URLSearchParams(window.location.search).get('diagnostic') ?? '')
  ...
  useEffect(() => {
    ...
    const unsubscribeRuntime = diagnosticMode ? runtime.subscribe(setPlayerDiagnostics) : () => undefined
    ...
    return () => { unsubscribe(); unsubscribeRuntime(); tvAudioService.dispose(); runtime.dispose() }
  ...
  const load = useCallback(async () => {
    ...
    if (programResult.error && playlistResult.error && callsResult.error) { runtime.error(programResult.error ?? playlistResult.error ?? callsResult.error); return }
    ...
    setPayload(next); setInterruptions(next.interruptions ?? []); savePayload(next); runtime.setCachedItems(next.items.length)
  ...
  useEffect(() => {
    ...
    let channel: ReturnType<typeof client.channel> | null = null; let reconnectTimer: number | undefined; let disposed = false; let attempts = 0
    const scheduleReconnect = () => { runtime.clear(reconnectTimer); const delay = Math.min(30_000, 1_500 * 2 ** attempts++); reconnectTimer = runtime.timeout(() => { if (!disposed) void connect() }, delay) }
    const disconnect = () => { const previous = channel; channel = null; runtime.setSubscriptions(0); if (previous) void client.removeChannel(previous) }
    const connect = async () => {
      if (disposed || document.hidden) return
      const previous = channel; channel = null; runtime.setSubscriptions(0)
      if (previous) await client.removeChannel(previous)
      if (disposed || document.visibilityState === 'hidden') return
      const nextChannel = client.channel(`tv:${companyId}:${displayId}:stable`).on('postgres_changes', { event: '*', schema: 'public', table: 'tv_programs', filter: `company_id=eq.${companyId}` }, () => void load()).on('postgres_changes', { event: '*', schema: 'public', table: 'tv_playlist_items', filter: `display_id=eq.${displayId}` }, () => void load()).on('postgres_changes', { event: '*', schema: 'public', table: 'tv_media', filter: `company_id=eq.${companyId}` }, () => void load()).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tv_displays', filter: `id=eq.${displayId}` }, () => void load()).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tv_interruptions', filter: `display_id=eq.${displayId}` }, () => void load()).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tv_calls', filter: `display_id=eq.${displayId}` }, () => void load())
      channel = nextChannel
      nextChannel.subscribe((status, error) => {
        if (disposed || channel !== nextChannel) return
        if (status === 'SUBSCRIBED') { attempts = 0; runtime.setSubscriptions(1); runtime.reconnected(); void load(); return }
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') { runtime.setSubscriptions(0); runtime.error(error ?? new Error(`Realtime: ${status}`)); scheduleReconnect() }
      })
    }
    reconnectRef.current = () => { if (!channel) void connect(); else void load() }
    disconnectRef.current = disconnect
    void connect()
    return () => { disposed = true; runtime.clear(reconnectTimer); disconnect(); reconnectRef.current = () => undefined; disconnectRef.current = () => undefined }
  ...
  useEffect(() => {
    const persist = () => savePlayback(companyId, displayId, { itemId: current?.id ?? '', itemIndex: index, elapsedSeconds: videoRef.current?.currentTime ?? 0, savedAt: new Date().toISOString() })
    const recover = () => { runtime.lifecycle('foreground'); if (supabase) void supabase.auth.refreshSession().catch(error => runtime.error(error)); reconnectRef.current(); void load(); void tvAudioService.resumeAudioContext(); const video = videoRef.current; if (video && current?.media.type === 'video') void video.play().catch(() => undefined) }
    const hide = () => { runtime.lifecycle('background'); persist(); tvAudioService.pauseAllAudio() }
    const visibility = () => { if (document.visibilityState === 'hidden') { hide(); disconnectRef.current() } else recover() }
    const pageHide = () => { runtime.lifecycle('pagehide'); hide(); disconnectRef.current() }
    const blur = () => { runtime.lifecycle('blur'); hide() }
    const offline = () => { runtime.lifecycle('offline'); runtime.error('Conexão de rede indisponível.'); disconnectRef.current() }
    window.addEventListener('pageshow', recover); window.addEventListener('pagehide', pageHide); window.addEventListener('focus', recover); window.addEventListener('blur', blur); window.addEventListener('online', recover); window.addEventListener('offline', offline); document.addEventListener('visibilitychange', visibility)
    const persistTimer = runtime.interval(persist, 5_000); const refreshTimer = runtime.interval(() => { if (!document.hidden) void load() }, 5 * 60_000)
    return () => { persist(); runtime.clear(persistTimer); runtime.clear(refreshTimer); window.removeEventListener('pageshow', recover); window.removeEventListener('pagehide', pageHide); window.removeEventListener('focus', recover); window.removeEventListener('blur', blur); window.removeEventListener('online', recover); window.removeEventListener('offline', offline); document.removeEventListener('visibilitychange', visibility) }
  }, [companyId, current?.id, current?.media.type, displayId, index, load, runtime])

  useEffect(() => {
    runtime.media(current?.media.title); progressRef.current = { itemId: current?.id ?? '', value: videoRef.current?.currentTime ?? index, changedAt: Date.now(), recoveries: 0 }
  }, [current?.id, current?.media.title, index, runtime])

  useEffect(() => {
    if (!next) { runtime.setPreloadCount(0); return }
    const url = resolveMediaUrl(next.media); if (!url) { runtime.setPreloadCount(0); return }
    let media: HTMLImageElement | HTMLVideoElement
    if (next.media.type === 'video') { const video = document.createElement('video'); video.preload = 'metadata'; video.muted = true; video.playsInline = true; video.src = url; media = video }
    else { const image = new Image(); image.decoding = 'async'; image.src = url; media = image }
    runtime.setPreloadCount(1)
    return () => { if (media instanceof HTMLVideoElement) { media.pause(); media.removeAttribute('src'); media.load() } else media.removeAttribute('src'); runtime.setPreloadCount(0) }
  }, [next, runtime])

  useEffect(() => {
    const watchdog = runtime.interval(() => {
      if (document.hidden || activeInterruption || !current) return
      const video = videoRef.current; const value = video && current.media.type === 'video' ? video.currentTime : index
      const progress = progressRef.current
      if (progress.itemId !== current.id || Math.abs(value - progress.value) > .2) { progressRef.current = { itemId: current.id, value, changedAt: Date.now(), recoveries: 0 }; return }
      const limit = current.media.type === 'video' ? 45_000 : (current.durationSeconds + 30) * 1000
      if (Date.now() - progress.changedAt < limit) return
      if (progress.recoveries < 2) { progress.recoveries += 1; progress.changedAt = Date.now(); runtime.error(`Watchdog recuperando mídia travada: ${current.media.title ?? current.id}`); void load(); if (video) { video.load(); void video.play().catch(() => undefined) } else setIndex(value => (value + 1) % Math.max(items.length, 1)); return }
      savePlayback(companyId, displayId, { itemId: current.id, itemIndex: index, elapsedSeconds: video?.currentTime ?? 0, savedAt: new Date().toISOString() }); runtime.controlledReload('Watchdog: player sem progresso após duas recuperações')
    }, 15_000)
    return () => runtime.clear(watchdog)
  }, [activeInterruption, companyId, current, displayId, index, items.length, load, runtime])
  ...
  }, [activated, activeInterruption, companyId, current?.id, displayId, index, interruptions, runtime])
  ...
  useEffect(() => {
    ...
    let spoken = false; let cancelled = false; let delayTimer: number | undefined
    const speak = async () => {
      if (spoken) return; spoken = true
      if (callSettings.bellEnabled) { try { await tvAudioService.playCallSound(); await new Promise<void>(resolve => { delayTimer = runtime.timeout(resolve, callSettings.bellDelayMs) }) } catch { /* diagnostics expose playback errors */ } }
      if (cancelled) return
    ...
    void speak()
    return () => { cancelled = true; runtime.clear(delayTimer); speechService.cancel() }
  ...
  useEffect(() => {
    ...
    const timer = runtime.timeout(() => setIndex(i => (i + 1) % items.length), current.durationSeconds * 1000)
    return () => runtime.clear(timer)
  ...
  if (!current) return <main className="tv-screen" aria-label="TV sem programação">{activeInterruption ? <CallOverlay interruption={activeInterruption}/> : null}{diagnosticMode ? <AudioDiagnostic diagnostics={audioDiagnostics} player={playerDiagnostics} soundEnabled={soundEnabled}/> : null}</main>

  return <main className="tv-screen"><Media key={current.id} item={current} displayId={displayId} videoRef={videoRef} soundEnabled={soundEnabled} onEnded={() => setIndex(i => (i + 1) % items.length)} onError={error => { runtime.error(error); void load() }} />{activeInterruption ? <CallOverlay interruption={activeInterruption}/> : null}{activationError ? <AudioUnlock onClick={activate} activating={activating}/> : null}{diagnosticMode ? <AudioDiagnostic diagnostics={audioDiagnostics} player={playerDiagnostics} soundEnabled={soundEnabled}/> : null}</main>
}
...
function AudioDiagnostic({ diagnostics, player, soundEnabled }: { diagnostics: TvAudioDiagnostics; player: TvPlayerDiagnostics; soundEnabled: boolean }) {
  const speech = speechService.diagnostics()
  return <aside className="audio-diagnostic"><strong>Diagnóstico do player</strong><span>áudio habilitado: {diagnostics.enabled ? 'sim' : 'não'}</span><span>sound_enabled: {soundEnabled ? 'true' : 'false'}</span><span>volume: {Math.round(diagnostics.volume * 100)}%</span><span>AudioContext: {diagnostics.contextState}</span><span>mídia carregada: {diagnostics.loadedMedia ?? 'nenhuma'}</span><span>última mídia: {player.lastMedia ?? 'nenhuma'}</span><span>recursos: {player.approximateResources}</span><span>subscriptions: {player.subscriptionCount}</span><span>timers gerenciados: {player.timerCount}</span><span>cache/preload: {player.cachedItems}/{player.preloadCount}</span><span>última reconexão: {player.lastReconnectAt ?? 'nenhuma'}</span><span>último evento: {player.lastLifecycleEvent ?? 'nenhum'}</span><span>último reload: {player.lastReloadReason ?? 'nenhum'}</span><span>voz: {speech.supported ? `${speech.voiceCount} disponível(is)` : 'indisponível'}</span><span>último erro: {player.lastError ?? diagnostics.lastError ?? speech.lastError ?? 'nenhum'}</span></aside>
}

function Media({ item, displayId, videoRef, soundEnabled, onEnded, onError }: { item: ProgramItem, displayId: string, videoRef: React.RefObject<HTMLVideoElement | null>, soundEnabled: boolean, onEnded: () => void, onError: (error: Error) => void }) {
  ...
  const attachVideo = useCallback((video: HTMLVideoElement | null) => {
    ...
    attachedVideo.current = video; videoRef.current = video
  }, [videoRef])
  useEffect(() => () => { if (attachedVideo.current) tvAudioService.releaseMedia(attachedVideo.current); videoRef.current = null }, [videoRef])
  ...
  return <div className="media-layer" style={{ '--media-fit': item.fit } as React.CSSProperties}>
    {item.media.type === 'video' && url ? <video ref={attachVideo} src={url} preload="auto" muted={!soundEnabled || item.muted} onLoadedMetadata={restoreAndPlay} onEnded={onEnded} onError={() => onError(new Error(`Falha ao carregar vídeo: ${item.media.title ?? item.id}`))} playsInline /> : null}
    {item.media.type === 'image' && url ? <img src={url} alt={item.media.title ?? ''} onError={() => onError(new Error(`Falha ao carregar imagem: ${item.media.title ?? item.id}`))} /> : null}
