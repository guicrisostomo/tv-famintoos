...
export function CallsPage({ companyId, displays, calls, loading, onReload }: { companyId: string; displays: TvDisplayRecord[]; calls: TvCallRecord[]; loading: boolean; onReload: () => Promise<void> }) {
  ...
  const clearableCount = calls.filter(call => call.status === 'completed' || call.status === 'cancelled').length
  ...
  const clearHistory = async () => {
    if (!supabase || clearableCount === 0 || !window.confirm('Limpar as chamadas concluídas e canceladas?')) return
    setClearing(true); setError(null)
    const { error: deleteError } = await supabase.from('tv_calls').delete().eq('company_id', companyId).in('status', ['completed', 'cancelled'])
