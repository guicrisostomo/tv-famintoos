...
export function PreviewPanel({ items }: { items: TvPlaylistRecord[] }) {
  ...
  return <section className="card"><div className="section-title"><h2>Pré-visualização 16:9</h2><span className="badge">{items.length > 0 ? `${index % items.length + 1}/${items.length}` : 'Sem itens'}</span></div><div className="preview">{!current ? <div className="preview-placeholder">Tela preta<br/>Nenhuma mídia válida configurada</div> : current.media_type === 'image' && imageUrl ? <img src={imageUrl} alt={current.title}/> : current.media_type === 'message' ? <div className="preview-message">{current.message_text}</div> : <div className="preview-placeholder">{current.title}</div>}</div><div className="preview-actions"><button className="button primary" disabled={items.length === 0} onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={15}/> : <Play size={15}/>} {playing ? 'Pausar prévia' : 'Reproduzir prévia'}</button><button className="button secondary" disabled={items.length < 2} onClick={next}><SkipForward size={15}/> Próximo item</button></div></section>
}
